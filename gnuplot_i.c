...
gnuplot_ctrl * gnuplot_init(void)
{
    ...
    handle->gnucmd = popen("gnuplot 2> ./log.txt", "w") ;
...
)
{
    ...
    if (title == NULL) {
        sprintf(line, "%s \"%s\" with %s lt rgb '#FF0000'", cmd, name, handle->pstyle);
    } else {
        sprintf(line, "%s \"%s\" title \"%s\" with %s lt rgb '#FF0000'", cmd, name,
                      title, handle->pstyle);
    }
...
)
{
    ...
    if (title == NULL) {
        sprintf(line, "%s \"%s\" notitle with %s %s\n", cmd, name, handle->pstyle,extras);
    } else {
        sprintf(line, "%s \"%s\" title \"%s\" with %s %s\n", cmd, name,
